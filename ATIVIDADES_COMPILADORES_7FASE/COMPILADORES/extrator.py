...
# Função para salvar os caracteres ASCII em um arquivo txt
def salvar_txt(caracteres_ascii, caminho_arquivo):
    with open(caminho_arquivo, 'w') as arquivo:
        arquivo.write("Caracteres ASCII extraídos\n")
        for char, ascii_val in caracteres_ascii:
            arquivo.write(f"{char} → {ascii_val}\n")
    print(f"Arquivo TXT salvo em: {caminho_arquivo}")

# Função para extrair caracteres ASCII de um texto
def extrair_caracteres_ascii(texto):
    return [(char, ord(char)) for char in texto]

# Função para abrir a caixa de diálogo e escolher o arquivo
def abrir_arquivo():
    caminho_arquivo = filedialog.askopenfilename(filetypes=[("Arquivos Word", "*.docx"), ("Arquivos PDF", "*.pdf"), ("Text files", "*.txt")])
    if caminho_arquivo:
        ...
        caracteres_ascii = extrair_caracteres_ascii(texto)
